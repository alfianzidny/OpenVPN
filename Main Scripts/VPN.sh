	...
	case $opt in
		...
		"PureVPN")
			echo "Running PureVPN"
				wget -O Pure.sh https://raw.githubusercontent.com/davesayers2014/OpenVPN/test/PureVPN/Pure.sh && sed -i "s/uuuu/$USERNAME/g" Pure.sh && sed -i "s/pppp/$PASSWORD/g" Pure.sh && chmod +x Pure.sh && ./Pure.sh				
			;;
		"PIA VPN")
