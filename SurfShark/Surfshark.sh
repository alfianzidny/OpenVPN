...
rm -rv /hdd/surfsharkvpn >/dev/null 2>&1
mkdir -p /etc/openvpn
...
echo $LINE
mkdir -p /hdd/surfsharkvpn
cd /hdd/surfsharkvpn
wget "https://account.surfshark.com/api/v1/server/configurations" -O /hdd/surfsharkvpn/tmp.zip; unzip /hdd/surfsharkvpn/tmp.zip; rm /hdd/surfsharkvpn/tmp.zip &> /dev/null 2>&1
...
init 4
sleep 3
sed -i '$i config.vpnmanager.directory=/hdd/Surfhark/' /etc/enigma2/settings
